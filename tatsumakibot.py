...
# Checking comment for "sassy lost child"
            if re.search(r"\b" + "sassy lost child" + r"\b", lowercase_comment) and parent.author != reddit.user.me() and comment.id not in cache and comment.author != reddit.user.me():
...
# Checking comment for "tatsu"
            if re.search(r"\b" + "tatsu" + r"\b", lowercase_comment) and parent.author != reddit.user.me() and comment.id not in cache and comment.author != reddit.user.me():
                comment.reply("It's Tatsumaki, not Tatsu you peasant!")
                print("Replied to comment" + comment.id)
                cache.append(comment.id)
# Checking comment for "brat"
            if re.search(r"\b" + "brat" + r"\b", lowercase_comment) and parent.author != reddit.user.me() and comment.id not in cache and comment.author != reddit.user.me():
...
# Checking comment for "spoilt brat"
            if re.search(r"\b" + "spoilt brat" + r"\b", lowercase_comment) and parent.author != reddit.user.me() and comment.id not in cache and comment.author != reddit.user.me():
...
# Checking comment for "best girl"
            if re.search(r"\b" + "best girl" + r"\b", lowercase_comment) and parent.author != reddit.user.me() and comment.id not in cache and comment.author != reddit.user.me():
...
# Checking comment for "tats"
            if re.search(r"\b" + "tats" + r"\b", lowercase_comment) and parent.author != reddit.user.me() and comment.id not in cache and comment.author != reddit.user.me():
                print('String with "tats" found.' + comment.id)
                comment.reply("It's **Tornado of Terror** to you, avocado head!")
                print("Replied to comment" + comment.id)
                cache.append(comment.id)
# Response to "bad bot"
            if re.search(r"\b" + "bad bot" + r"\b", lowercase_comment) and comment.id not in cache and parent.author == reddit.user.me():
...
# Response to username call
            if re.search(r"\b" + "u/Not_a_brat_Tatsumaki" + r"\b", lowercase_comment) and comment.id not in cache:
                ...
                cache.append(comment.id)
# Linking Github
            if "who created you" in lowercase_comment and comment.id not in cache and parent.author == reddit.user.me():
